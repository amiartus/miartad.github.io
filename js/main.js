...
function db_create_cb(event) {
  ...
  store = db.createObjectStore(table_history, {
    keyPath: "id",
    autoIncrement: true,
  })
...
function db_get(table, index, value) {
  return db_request(table, 1, function (db) {
    ...
    else return db.transaction(table).objectStore(table).index(index).get(value)
  })
}
...
async function save_task(task_name, task_start, task_end) {
  ...
  let t = {
    ...
    [key_week]: getWeekNumber(new Date(task_start)),
...
async function update_history_ui() {
  ...
  let results = await db_foreach(
    table_history,
    key_start,
    IDBKeyRange.lowerBound(day_start.getTime(), true)
  )
  ...
  results.forEach((element) => {
    ...
    fields.forEach((field) => {
      ...
      if (field == key_start) val = new Date(val).toTimeString().split(" ")[0]
...
async function update_progress_ui() {
  ...
  document.getElementById("task-update").value = toDuration(runtime)
}
...
document
  ...
  .addEventListener("focusout", function (ev) {
    if (ev.target.value == "") update_task_ui()
  })

//------------------------------------------------------------------------------------------------------------------
document.getElementById("dummy-test").addEventListener("click", runTest)
